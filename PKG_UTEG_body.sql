...
SSRMEET_MON_DAY Lunes,
SSRMEET_TUE_DAY Martes,
SSRMEET_WED_DAY Mi�rcoles,
SSRMEET_THU_DAY Jueves,
SSRMEET_FRI_DAY Viernes,
SSRMEET_SAT_DAY S�bado,
to_char(to_date(SSRMEET_BEGIN_TIME, 'hh24mi'),'hh24:mi')h_inicio,
to_char(to_date(SSRMEET_END_TIME,'hh24mi'),'hh24:mi') h_fin,
SLBRDEF_ROOM_NUMBER salon,
STVBLDG_DESC edificio,
SLBRDEF_DESC ubicacion
FROM SIRASGN, SSRMEET, SSBSECT,SCBCRSE,STVBLDG,SLBRDEF
WHERE 1=1 AND SIRASGN_PIDM = (SELECT SPRIDEN_PIDM FROM SPRIDEN WHERE SPRIDEN_ID=iden AND SPRIDEN_CHANGE_IND IS NULL)
